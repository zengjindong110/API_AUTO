...
def get_value_type(respond):
    ...
    if type(respond) == dict:
        ...
        for key in keys:
            key_value_type[key] = type(respond[key])
    ...
    for key in respond.keys():
        ...
        if type(respond[key]) == dict and respond[key]:
            ...
            get_value_type(respond[key])

        elif type(respond[key]) == list and respond[key]:

            if type(respond[key][0]) != str:
                get_value_type(respond[key][0])
            else:
                pass
        else:
            pass
...
if __name__ == '__main__':
    ...
    # a = get_value_type(data)
    # print(a)
    # print(get_key(data))
    first_keys = []
    x = map(lambda first_key: first_keys.append(first_key),[1])
    print(list(x))
    print(first_keys)

    s = [1, 2, 3]
    c =  list(map(lambda x, y, z: x * y * z, s, s, s))
    print(c)